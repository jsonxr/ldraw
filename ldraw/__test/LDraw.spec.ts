...
import { LoadFile } from '../src/loaders/LoadFile';
...
describe('LDraw', () => {
  ...
  describe('.load', () => {
    it('should return null if it can not load file', async () => {
      ldraw.loaders = [jest.fn().mockResolvedValue(undefined)];
    ...
    it('should return null and not cache if file loaded is not an LDrawFile', async () => {
      ...
      ldraw.loaders = [jest.fn().mockResolvedValue('xyz fake file')];
      expect.assertions(1);
    ...
    it('should return parsed file if it finds', async () => {
      ...
      expect(ldraw.loaders[0]).toHaveBeenNthCalledWith(2, 'parts/819.dat');
    });
    ...
    it('should return null if it receives empty file from loadFile', async () => {
      ldraw.loaders = [jest.fn().mockResolvedValue('\n\n')];
      const file = await ldraw.load('fake.dat');
  ...
  describe('.find', () => {
    it('should not care if folder has a / on end or not', async () => {
      ldraw = new LDraw({
        folders: ['parts', 'p/'],
        loaders: [jest.fn().mockReturnValue(null)],
      });
      await ldraw.find('a.dat');
      expect(ldraw.loaders[0]).toHaveBeenNthCalledWith(1, 'parts/a.dat');
      expect(ldraw.loaders[0]).toHaveBeenNthCalledWith(2, 'p/a.dat');
    });
    it('should convert "\\" to "/"', async () => {
      ...
      await ldraw.find('a.dat');
      expect(ldraw.loaders[0]).toHaveBeenCalledWith('library/parts/a.dat');
    });
  });
});
